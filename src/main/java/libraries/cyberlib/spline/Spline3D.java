...
/**
 * Represents a three-dimensional parametric spline that interpolates between two
 * points and an orientation.
 */
public abstract class Spline3D {
    ...
    /**
     * Returns the ControlVector of the spline at time t
     *
     * @return The ControlVector of the spline at time t
     */
    public ControlVector getControlVector(double t) {
        ...
        final double z = combined.get(2, 0);
        ...
        if (t == 0) {
            dx = coefficients.get(3, m_degree - 1);
            dy = coefficients.get(4, m_degree - 1);
            dz = coefficients.get(5, m_degree - 1);
            ddx = coefficients.get(6, m_degree - 2);
            ddy = coefficients.get(7, m_degree - 2);
            ddz = coefficients.get(8, m_degree - 2);
        } else {
            // Divide out t once for first derivative.
            dx = combined.get(3, 0) / t;
            dy = combined.get(4, 0) / t;
            dz = combined.get(5, 0) / t;

            // Divide out t twice for second derivative.
            ddx = combined.get(6, 0) / t / t;
            ddy = combined.get(7, 0) / t / t;
            ddz = combined.get(8, 0) / t / t;
        }
    ...
    /**
     * Represents a control vector for a 3D spline.
     *
    ...
    public static class ControlVector {
         ...
         * @param z The z dimension of the control vector.
         */
        @SuppressWarnings("ParameterName")
