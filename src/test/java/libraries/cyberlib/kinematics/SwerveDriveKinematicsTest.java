...
class SwerveDriveKinematicsTest {
  ...
  void testTurnInPlaceInverseKinematics() {
    ...
    assertAll(
        () -> assertEquals(106.63, moduleStates[0].speedInMetersPerSecond, 0.1),
        () -> assertEquals(106.63, moduleStates[1].speedInMetersPerSecond, 0.1),
        () -> assertEquals(106.63, moduleStates[2].speedInMetersPerSecond, 0.1),
        () -> assertEquals(106.63, moduleStates[3].speedInMetersPerSecond, 0.1),
  ...
  void testTurnInPlaceForwardKinematics() {
    ...
    assertAll(
        () -> assertEquals(0.0, chassisSpeeds.vxInMetersPerSecond, kEpsilon),
        () -> assertEquals(0.0, chassisSpeeds.vyInMetersPerSecond, kEpsilon),
        () -> assertEquals(2 * Math.PI, chassisSpeeds.omegaInRadiansPerSecond, 0.1)
    );
  }
  ...
  void testOffCenterCORRotationInverseKinematics() {
    ...
    */
    ...
    assertAll(
        () -> assertEquals(0.0, moduleStates[0].speedInMetersPerSecond, 0.1),
        () -> assertEquals(150.796, moduleStates[1].speedInMetersPerSecond, 0.1),
        () -> assertEquals(150.796, moduleStates[2].speedInMetersPerSecond, 0.1),
        () -> assertEquals(213.258, moduleStates[3].speedInMetersPerSecond, 0.1),
  ...
  void testOffCenterCORRotationForwardKinematics() {
    ...
    */
    ...
    assertAll(
        () -> assertEquals(75.398, chassisSpeeds.vxInMetersPerSecond, 0.1),
        () -> assertEquals(-75.398, chassisSpeeds.vyInMetersPerSecond, 0.1),
        () -> assertEquals(2 * Math.PI, chassisSpeeds.omegaInRadiansPerSecond, 0.1)
    );
  }
  ...
  private void assertModuleState(SwerveModuleState expected, SwerveModuleState actual,
    ...
    assertAll(
        () -> assertEquals(expected.speedInMetersPerSecond, actual.speedInMetersPerSecond,
            tolerance.speedInMetersPerSecond),
  ...
  void testOffCenterCORRotationAndTranslationForwardKinematics() {
    ...
    */
    ...
    assertAll(
        () -> assertEquals(0.0, chassisSpeeds.vxInMetersPerSecond, 0.1),
        () -> assertEquals(-33.0, chassisSpeeds.vyInMetersPerSecond, 0.1),
        () -> assertEquals(1.5, chassisSpeeds.omegaInRadiansPerSecond, 0.1)
    );
  }
  ...
  void testNormalize() {
    ...
    assertAll(
        () -> assertEquals(5.0 * factor, arr[0].speedInMetersPerSecond, kEpsilon),
        () -> assertEquals(6.0 * factor, arr[1].speedInMetersPerSecond, kEpsilon),
        () -> assertEquals(4.0 * factor, arr[2].speedInMetersPerSecond, kEpsilon),
        () -> assertEquals(7.0 * factor, arr[3].speedInMetersPerSecond, kEpsilon)
    );
  }

  @Test
  void ListConstructor() {
    List<Translation2d> modules = new ArrayList<>();
    modules.add(m_fr);
    modules.add(m_fl);
    modules.add(m_bl);
    modules.add(m_br);

    SwerveDriveKinematics kinematics = new SwerveDriveKinematics(modules);

    SwerveModuleState state = new SwerveModuleState(5.0, Rotation2d.fromDegrees(90.0));
    var chassisSpeeds = kinematics.toChassisSpeeds(state, state, state, state);
